...
- Type-check with `mypy` (strict mode is enabled for this package).
...
`to_zenodo_record`) on `NeuralAvalancheUTAC`. Any change to these methods'
...
scientific model, prediction, or benchmark (e.g. the Γ_brain ≈ 0.251
cross-domain universality result, UTAC parameters, falsifiable
predictions), please cite the source and clearly mark speculative vs.
validated claims.
